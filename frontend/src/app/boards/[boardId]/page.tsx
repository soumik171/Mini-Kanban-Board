...
const BOARD_TITLE_SESSION = "board-title" as const;
...
function BoardContent() {
  ...
  // it owns the lock, so claiming by another session (e.g. the display name in
  // the app header) closes it automatically.
  const isRenamingBoard = active === BOARD_TITLE_SESSION;

  // Never leave the lock held after this component unmounts.
  useEffect(() => () => release(BOARD_TITLE_SESSION), [release]);
  ...
  function startRenamingBoard() {
    if (!detail) return;
    setBoardTitle(detail.board.title);
    claim(BOARD_TITLE_SESSION);
  }

  function stopRenamingBoard() {
    release(BOARD_TITLE_SESSION);
  }

  async function handleRenameBoard(event: FormEvent) {
    event.preventDefault();
    if (!detail) return;
    const title = boardTitle.trim();
    if (!title || savingBoardTitle) return;
    setSavingBoardTitle(true);
    try {
      await updateBoard(detail.board.id, { title });
      stopRenamingBoard();
      void load();
    } catch (err) {
      setActionError(err instanceof ApiError ? err.message : "Could not rename board");
    } finally {
      ...
      <div className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
          <div className="min-w-0">
            ...
            <div className="group mt-1 flex items-center gap-3">
              {isRenamingBoard && canEdit ? (
                <form onSubmit={handleRenameBoard} className="flex min-w-0 items-center gap-1.5">
                  <input
                    type="text"
                    autoFocus
                    value={boardTitle}
                    ...
                    maxLength={120}
                    className="w-64 rounded-md border border-slate-300 px-2 py-1 text-lg font-semibold text-slate-900 outline-none focus:border-indigo-500"
                  />
                  <button
                    type="submit"
                    aria-label="Save board title"
                    title="Save"
                    disabled={savingBoardTitle || !boardTitle.trim()}
                    className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-indigo-600 text-white transition hover:bg-indigo-700 disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    aria-label="Cancel"
                    title="Cancel"
                    onClick={stopRenamingBoard}
                    className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-200/70 hover:text-slate-600"
                  >
                    <svg viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
                ...
                <>
                  ...
                  {canEdit ? (
                    <button
                      ...
                      className="shrink-0 rounded p-1 text-slate-400 opacity-0 transition group-hover:opacity-100 hover:bg-slate-200/70 hover:text-slate-600"
                    >
                      <svg viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
